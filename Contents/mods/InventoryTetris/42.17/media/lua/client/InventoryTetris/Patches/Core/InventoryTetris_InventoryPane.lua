...
Events.OnGameBoot.Add(function()
    ...
    function ISInventoryPane:onMouseUp(x, y)
        ...
        -- Handle drag-and-drop transfers when items are dropped on the pane but not on a specific grid
        if DragAndDrop.isDragging() then
            ...
            if stacks then
                ...
                for _, stack in ipairs(stacks) do
                    local frontItem = stack.items and stack.items[1]
                    if frontItem and frontItem:getContainer() ~= self.inventory then
                        for i=2, #stack.items do
                            local item = stack.items[i]
                            local transfer = ISInventoryTransferAction:new(playerObj, item, item:getContainer(), self.inventory)
